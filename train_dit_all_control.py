...
from transformers import (
    ...
    CLIPTextModel,
    T5EncoderModel,
    AutoProcessor, SiglipVisionModel,
)
...
from dataset_360all import AigcDataset
from dataset_extract300w import AigcExtractDataset
...
def make_train_dataset(args, clip_image_processor, accelerator):
    dataset=AigcExtractDataset(args, clip_image_processor=clip_image_processor)
    with accelerator.main_process_first():
...
def collate_fn(examples):
    ...
    image_prompt = torch.stack([example[2].squeeze(0) for example in examples])
    image_prompt = image_prompt.to(memory_format=torch.contiguous_format).float()

    clip = torch.stack([example[3].squeeze(0) for example in examples])
    clip = clip.to(memory_format=torch.contiguous_format).float()
    t5 = torch.stack([example[4].squeeze(0) for example in examples])
    t5 = t5.to(memory_format=torch.contiguous_format).float()

    return img, prompt, image_prompt, clip, t5


class IPAdapter_FLUX(torch.nn.Module):
    _tied_weights_keys = None
    def __init__(self, adapter_modules=None, project_model=None, ckpt_path=None):
        super().__init__()
    ...
    def forward(self, dit, project_embeds, **kwarg):
        project_embeds=self.project_model(project_embeds)
        ...
        cond_ids = repeat(cond_ids, "h w c -> b (h w) c", b=kwarg['txt_ids'].shape[0])

        bdm_ids = cond_ids.to(dtype=kwarg['txt_ids'].dtype, device=kwarg['txt_ids'].device)
        noise_pred = dit(image_emb=project_embeds, bdm_ids=bdm_ids, **kwarg)
        return noise_pred
...
def build_fsdp_plugin():
    fsdp_plugin = FullyShardedDataParallelPlugin(
        ...
        auto_wrap_policy=ModuleWrapPolicy([IPAdapter_FLUX,MLPProjModel]),
...
def _encode_prompt_with_mt5(
    ...
    prompt_embeds = prompt_embeds.hidden_states[-1] # [2, 300, 4096]
...
def main(args):
    ...
    dist.init_process_group(
        backend='nccl',
        init_method='env://', 
        timeout=datetime.timedelta(minutes=60)  # 设置为30分钟
    )
    ...
    flux_transformer.to(accelerator.device, dtype=torch.bfloat16)
            # load image encoder
    image_encoder.to(accelerator.device, dtype=torch.bfloat16)
    ...
    for name in flux_transformer.attn_processors.keys():
        if name.startswith("transformer_blocks.") or name.startswith("single_transformer_blocks"):
            ...
            ).to(flux_transformer.device, dtype=torch.float32)
        else:
            print(name)
            ip_attn_procs[name] = flux_transformer.attn_processors[name]

    flux_transformer.set_attn_processor(ip_attn_procs)
    #adapter_modules = torch.nn.ModuleList(flux_transformer.attn_processors.values())   #(flux_transformer.attn_processors.values())
    adapter_modules = torch.nn.ModuleList( p for p in flux_transformer.attn_processors.values() if isinstance(p, IPAFluxAttnProcessor2_0))
    # adapter_modules = torch.nn.ModuleList( p for p in flux_transformer.attn_processors.values() if isinstance(p,IPAFluxAttnProcessor2_0))
    project_model = MLPProjModel(
        cross_attention_dim=flux_transformer.config.joint_attention_dim, # 4096
        id_embeddings_dim=1152,
        num_tokens=num_tokens,
    ).to(flux_transformer.device, dtype=torch.float32)
    # if torch.distributed.get_rank() == 0:  # 只在主卡上触发
    #     breakpoint()
    flux_ipa = IPAdapter_FLUX(adapter_modules, project_model).to(flux_transformer.device, dtype=torch.float32)
    ...
    if args.gradient_checkpointing:
        ...
        flux_ipa.enable_gradient_checkpointing()
    ...
    # Optimizer creation
    params_to_optimize = flux_ipa.parameters()
    total_params = sum(p.numel() for p in flux_ipa.parameters() if p.requires_grad)
    total_params2 = sum(p.numel() for p in flux_ipa.parameters())
    print(f"total_params:{total_params}")
    print(f"total_params2:{total_params2}")
    ...
    # if torch.distributed.get_rank() == 0:  # 只在主卡上触发
    #     breakpoint()
    flux_ipa, optimizer, lr_scheduler = accelerator.prepare(
        flux_ipa, optimizer, lr_scheduler
    )
    ...
    print(sum([p.numel() for p in flux_ipa.parameters() if p.requires_grad]) / 1000000, 'parameters')
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(flux_ipa):
                #import pdb;pdb.set_trace()
                img, prompt, image_prompt, clip, t5 = batch
                #import pdb;pdb.set_trace()
                # if torch.distributed.get_rank() == 6:  # 只在主卡上触发
                #     breakpoint()
                # Convert images to latent space
                # vae encode to(device=accelerator.device, dtype=dtype)
                pixel_values = img.to(device=accelerator.device, dtype=weight_dtype)
                ...
                # )
                prompt_embeds = t5.to(device=pixel_latents.device, dtype=pixel_latents.dtype)
                pooled_prompt_embeds = clip.to(device=pixel_latents.device, dtype=pixel_latents.dtype)
                text_ids = torch.zeros(prompt_embeds.shape[1], 3).to(device=pixel_latents.device, dtype=pixel_latents.dtype)
                ...
                #import pdb;pdb.set_trace()
                clip_image_embeds = image_encoder(image_prompt.to(image_encoder.device, dtype=image_encoder.dtype)).pooler_output

                noise_pred = flux_ipa(
                    dit=flux_transformer,
                ...
                for name, param in flux_ipa.named_parameters():
                    if param.grad is not None and torch.isnan(param.grad).any():
                        logger.error(f"Gradient for {name} contains NaN!")

                if accelerator.sync_gradients:
                    params_to_clip = flux_ipa.parameters()
            ...
            if accelerator.sync_gradients:
                ...
                if global_step!=0 and global_step!=initial_global_step and global_step % args.checkpointing_steps == 0:
    ...
    if accelerator.is_main_process:
        flux_controlnet = unwrap_model(flux_ipa)
